        ...
        if ($allow_customerimport == 1) {
            try {
                ...
                foreach ($clientsrezo as $item) {
                    ...
                    $output .= 'api_customerid: ' . $api_customerid . '\n';
                    $customer_id = Db::getInstance()->getValue('select system_customerid from ' . _DB_PREFIX_ .
                    ...
                    if (!$found) {
                        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            $output .= parent::l('Email') . ' : ' . $email . '\n';
                            $output .= parent::l('Firstname') . ' : ' . $firstname . '\n';
                            $output .= parent::l('Lastname') . ' : ' . $lastname . '\n';
                            ...
                            $output .= parent::l('Customer added') . ' : ' . $email . '\n';
                            ...
                            $output .= parent::l('Address 1') . ' : ' . $address->address1 . '\n';
                            $output .= parent::l('City') . ' : ' . $address->city . '\n';
                            try {
                                $address->add();
                            } catch (Exception $e) {
                                $output .= Vccsv::logError($e);
                            }
                            $output .= parent::l('Address created') . ' : ' . $address->id . '\n';
                        } else {
                            $output .= parent::l('Email incorrect') . ' : ' . $customer_id . '\n';
                        }
                    } else {
                        ...
                        $customer->newsletter = $newsletter;
                        $customer->update();
                        $output .= parent::l('Customer updated') . ' : ' . $customer_id . '\n';
                    }
                }
            } catch (SoapFault $exception) {
                $output .= Vccsv::logError($exception);
